...
``setlocale`` fonksiyonu aslında standart bir C fonksiyonudur. (Her standart C fonksiyonunun aynı zamanda bir POSIX
fonksiyonu durumunda olduğunu anımsayonız.) Prototipi şöyledir:
...
Fonksiyon önce argüman olarak girilen yazıyı, sonra bu yazının hemen yanına ``:`` ve SPACE karakterini, 
sonra da o andaki ``errno`` değerinin yazısını ve ```\n``` karakterini ``stderr`` dosyasına yazdırmaktadır. 
Bu fonksiyonun aşağıdaki gibi yazıldığını varsayabilirsiniz:
...
Bazı programcılar ``exit_sys`` işlevini gören fonksiyonu ``printf`` fonksiyonuna benzeterek de yazmaktadır. Örneğin Stevens,
